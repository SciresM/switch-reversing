...
from hashes import all_hashes, all_hashes_300, known_interface_map
...
def get_interface_id(process_function_string):
    ...
    #   MOVK W?, #?,LSL#16
    #   STP  W?, W?, [X?, #8]
    regex = '|'.join(re.escape(chr(0x60 | i) + '\xCA\x88\x52' + chr(0x60 | i) + '\xE8\xA9\x72') for i in range(29))
    sfco_const = re.findall(regex, process_function_string)
    if len(sfco_const) == 0:
        return 0
    process_function_string = process_function_string[:process_function_string.index('\xC0\x03\x5F\xD6', process_function_string.index(sfco_const[0]))]
    sfco_const = re.findall(regex, process_function_string)
    if len(sfco_const) != 1:
        return 0
    process_function_string = process_function_string[process_function_string.index(sfco_const[0]):]
    sfco_reg = ord(process_function_string[0]) & 0x1F
...
def dump_ipc_filename(fname):
    ...
    if not process_functions:
        ...
        for i in re.finditer(regex, text_string):
            ...
            retaddr_idx = text_string.index('\xC0\x03\x5F\xD6', process_function)
